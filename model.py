...
class Attention(nn.Module):
    ...
    def _attn(self, q, k, v, attention_mask=None, head_mask=None, autoregr_mask=True):
        w = torch.matmul(q, k)
        if self.scale:
    ...
    def forward(self, x, layer_past=None, attention_mask=None, head_mask=None, enc_hidden=None):
        ...
        attn_outputs = self._attn(query, key, value, attention_mask, head_mask, autoregr_mask)
        a = attn_outputs[0]
...
class GPT2Model(GPT2PreTrainedModel):
    def __init__(self, config):
        ...
        self.h = nn.ModuleList([Block(config.n_ctx, config, scale=True, encoder_attn=True) for _ in range(config.n_layer)])
    ...
    Examples::
        ...
        for i, (block, layer_past) in enumerate(zip(self.h, past)):
            ...
            outputs = block(
                hidden_states, layer_past=layer_past, attention_mask=attention_mask, head_mask=head_mask[i], encoder_hidden=encoder_hidden
            )
...
class FullModel(nn.Module):
    ...
    def process_story(self, story, max_context=256, random=True):
        max_context = min(max([len(i) for i in story]), max_context)
        story = [[self.decoder_tokenizer.encoder['<|endoftext|>']] + self.decoder_tokenizer.encode(i) + [self.decoder_tokenizer.encoder['<|endoftext|>']] for i in story]

        if random:
            story = [random_truncate(i, max_context) for i in story]
        else:
            story = [i[:max_context] for i in story]
...
if __name__ == '__main__':
    ...
    example_prompt = "Reddit buys the moon"

    tokenizer = model.encoder_tokenizer

    prompt_tokens = tokenizer.tokenize(example_prompt)
    ...
    input_ids = [tokenizer.convert_tokens_to_ids(prompt_tokens) for i in range(BATCH_SIZE)]
    input_ids = pad_sequences(input_ids, maxlen=MAX_LEN, dtype="long", truncating="post", padding="post")
    masks = [i>0 for i in input_ids]
    ...
    encoded, _ = model.encoder(input_ids, token_type_ids=None, attention_mask=masks)
    ...
    print(tokenizer.tokenize(example_prompt))

    dec_input = torch.LongTensor([tokenizer.encode(example_prompt) for i in range(BATCH_SIZE)]).cuda()
    ...
    loss.backward()
    """
    out, logprobs = sample_sequence(model.decoder, 100, encoder_hidden=encoder_hidden, batch_size=BATCH_SIZE, context=tokenizer.encode(example_prompt), eos_token=tokenizer.encoder['<|endoftext|>'], 
